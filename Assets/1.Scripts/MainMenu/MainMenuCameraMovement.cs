...
public class MainMenuCameraMovement : MonoBehaviour
{
    ...
    IEnumerator SelectCharacter()
    {
        while (true)
        {
            ...
            if (GameManager.Instance != null && GameManager.Instance.travelersEnabled.Count > 0 && GameManager.Instance.adventurersEnabled != null)
            {
                int t = 0;
                if (GameManager.Instance.travelersEnabled[t = UnityEngine.Random.Range(0, GameManager.Instance.travelersEnabled.Count)] != null)
                {
                    yield return StartCoroutine("ChasingCharacter", GameManager.Instance.travelersEnabled[t]);
                }
            }
        }
    }
